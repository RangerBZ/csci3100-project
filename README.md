...
## Side notes
- The production is deployed on render (for consistent database use, and image display function, costing 13 USD$ in total for our group. (potenially also for SendGrid).
- The test coverage differs little on local production and githubs setting, possible due to simple-cov versions variations.
- We modify the readme and .env after the ddl as we find that providing SendGrid API key to public for local test/github actions will disable the current key, so you can only use the email function in deployed production link.
- The SendGrid API offcial sending requires a formal domain name, this project only uses single sender API, so it comes from personal .gmail address.
Possibly needs to look up in trash mail folder. Email send might take up to 5 minutes. (The sendGrid may throw 404 in our tests for continous sends / invalid address / exceeds respond ratio).
